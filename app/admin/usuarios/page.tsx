...
import { getUsuariosSoporte, createUsuarioSoporte, updateUsuarioSoporte, deleteUsuarioSoporte } from '@/lib/supabase';
...
export default function UsuariosPage() {
  ...
  const handleEliminarUsuario = async (usuario: UsuarioSoporte) => {
    if (!confirm(`¿Está seguro de que desea eliminar permanentemente al usuario ${usuario.nombre_completo}? Esta acción no se puede deshacer.`)) {
      return;
    }

    try {
      await deleteUsuarioSoporte(usuario.id);
      await cargarUsuarios();
                ...
                <div key={usuario.id} className="p-6 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${usuario.activo
